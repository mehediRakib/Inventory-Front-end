...
import {DeleteSupplierRequest, SupplierListRequest} from "../../APIRequest/SuppliersAPIRequest.js";
...
const SupplierList = ()=> {
    ...
    useEffect( () => {
        (async () => {
            await SupplierListRequest(1, perPage, searchArray);
        })()

    }, []);

    const ListOfSupplier=useSelector((state)=>(state.supplier.List));
    const TotalSupplierList=useSelector((state)=>state.supplier.supplierListTotal);

    console.log("listOfSupplier",ListOfSupplier)
        console.log('total',TotalSupplierList)

    const handlePageClick=async (e) => {
        await SupplierListRequest(e.selected+1,perPage,searchArray);
    }
    const perPageOnChange=async (e) => {
        setPerPage(parseInt(e.target.value))
        await SupplierListRequest(1,e.target.value,searchArray)
    }
    ...
    const doDelete=async (id)=>{
        ...
        if(res.isConfirmed){
            const result=await DeleteSupplierRequest(id);
            if(result){
                await SupplierListRequest(1, perPage, searchArray);
            }
                                        ...
                                        <tr className="bg-white border-b">
                                            ...
                                            <td className="px-4 py-2">
                                                <div className="flex space-x-5 items-center">
                                                   <Link to={`/Create-Update-Supplier?id=${item['_id']}`}>
                        ...
                        <div className="mt-10 ">
                            <nav aria-label="Page navigation">
                                <ReactPaginate
                                    ...
                                    pageCount={Math.ceil(TotalSupplierList / perPage)}
