...
import {SalesApiRequest} from "../../APIRequest/SalesAPIRequest.js";
...
const SalesList =()=> {
    ...
    useEffect( () => {
        ...
        (async () => {
            await SalesApiRequest(1, perPage, searchArray);
        })()

    }, []);

    const TotalSalesList=useSelector((state)=>(state.sale.TotalSaleList));
    const ListOfSales=useSelector((state)=>state.sale.List);

    const handlePageClick=async (e) => {
        await SalesApiRequest(e.selected+1,perPage,searchArray);
    }
    const perPageOnChange=async (e) => {
        setPerPage(parseInt(e.target.value))
        await SalesApiRequest(1,e.target.value,searchArray)
    }
    const searchKeywordOnChange=async (e)=>{
        setSearchArray(e.target.value);
        if((e.target.value).length===0){
            setSearchArray('0');
            await SalesApiRequest(1,perPage,'0');
        }
    }
                                        ...
                                        <tr className="bg-white border-b">
                                            ...
                                            <td className="px-4 py-2">
                                                <div className="flex space-x-5 items-center">
                                                    <Link to={`/SalesCreateUpdate?id=${item['_id']}`}>
                        ...
                        <div className="mt-10">
                            <nav aria-label="Page navigation">
                                <ReactPaginate
                                    ...
                                    pageCount={Math.ceil(TotalSalesList / perPage)}
