...
import {DeleteProductRequest, ProductApiRequest} from "../../APIRequest/ProductAPIRequest.js";
...
const ProductList =()=> {
    ...
    useEffect( () => {
        ...
        (async () => {
            await ProductApiRequest(1, perPage, searchArray);
        })()

    }, []);

    const TotalProductList=useSelector((state)=>(state.product.TotalProductList));
    const ListOfProduct=useSelector((state)=>state.product.List);

    const handlePageClick=async (e) => {
        await ProductApiRequest(e.selected+1,perPage,searchArray);
    }
    const perPageOnChange=async (e) => {
        setPerPage(parseInt(e.target.value))
        await ProductApiRequest(1,e.target.value,searchArray)
    }
    const searchKeywordOnChange=async (e)=>{
        setSearchArray(e.target.value);
        if((e.target.value).length===0){
            setSearchArray('0');
            await ProductApiRequest(1,perPage,'0');
        }
    }
...

                                {
                                    ListOfProduct.map((item,index)=>
                                        <tr className="bg-white border-b">
                                            ...
                                            <td className="px-4 py-2"> {item['brand'][0]['name']} </td>
                                            <td className="px-4 py-2"> {item['category'][0]['name']} </td>
                                            <td className="px-4 py-2">{item['details']}</td>
                                            <td className="px-4 py-2">
                                                <div className="flex space-x-5 items-center">
                                                    <Link to={`/CreateUpdateProductList?id=${item['_id']}`}>
                        ...
                        <div className="mt-10">
                            <nav aria-label="Page navigation">
                                <ReactPaginate
                                    ...
                                    pageCount={Math.ceil(TotalProductList / perPage)}
