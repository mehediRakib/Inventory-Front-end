...
const CategoryList = () => {
    ...
    useEffect( () => {
        (async () => {
            await CategoryListRequest(1, perPage, searchArray);
        })()

    }, []);

    const ListOfCategory=useSelector((state)=>(state.category.List));
    const TotalCategory=useSelector((state)=>state.category.categoryListTotal);

    const handlePageClick=async (e) => {
        await CategoryListRequest(e.selected+1,perPage,searchArray);
    }
    const perPageOnChange=async (e) => {
        setPerPage(parseInt(e.target.value))
        await CategoryListRequest(1,e.target.value,searchArray)
    }
                                        ...
                                        <tr className="bg-white border-b">
                                            ...
                                            <td className="px-4 py-2">
                                                <div className="flex space-x-5 items-center">
                                                   <Link to={`/CreateUpdate-CategoryList?id=${item['_id']}`}>
                        ...
                        <div className="mt-10 ">
                            <nav aria-label="Page navigation">
                                <ReactPaginate
                                    ...
                                    pageCount={Math.ceil(TotalCategory / perPage)}
