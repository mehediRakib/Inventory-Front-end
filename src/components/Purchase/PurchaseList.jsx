...
import {PurchaseApiRequest} from "../../APIRequest/PurchaseAPIRequest.js";
...
const PurchaseList =()=> {
    ...
    useEffect( () => {
        ...
        (async () => {
            await PurchaseApiRequest(1, perPage, searchArray);
        })()

    }, []);

    const TotalPurchaseList=useSelector((state)=>(state.purchase.TotalPurchaseList));
    const ListOfPurchase=useSelector((state)=>state.purchase.List);

    const handlePageClick=async (e) => {
        await PurchaseApiRequest(e.selected+1,perPage,searchArray);
    }
    const perPageOnChange=async (e) => {
        setPerPage(parseInt(e.target.value))
        await PurchaseApiRequest(1,e.target.value,searchArray)
    }
    const searchKeywordOnChange=async (e)=>{
        setSearchArray(e.target.value);
        if((e.target.value).length===0){
            setSearchArray('0');
            await PurchaseApiRequest(1,perPage,'0');
        }
    }
...

                                {
                                    ListOfPurchase.map((item,index)=>
                                        <tr className="bg-white border-b">
                                            ...
                                            <td className="px-4 py-2">{item['Suppliers'][0]['supplierName']}</td>
                                            ...
                                            <td className="px-4 py-2">
                                                <div className="flex space-x-5 items-center">
                                                    <Link to={`/CreatePurchase?id=${item['_id']}`}>
                        ...
                        <div className="mt-10">
                            <nav aria-label="Page navigation">
                                <ReactPaginate
                                    ...
                                    pageCount={Math.ceil(TotalPurchaseList / perPage)}
