...
import { PurchaseReportByDateRequest} from "../../APIRequest/ReportAPIRequest.js";

const PurchaseReport = ()=> {
    const FromDateRef = useRef();
    const ToDateRef = useRef();

    const PurchaseListByDate=useSelector((state)=>state.Report.PurchaseByDateList)
    const CreateReport = async () => {
        ...
        if (isEmpty(FromDate)) {
            ...
            await PurchaseReportByDateRequest(FromDate,ToDate);
            ...
            <div className="container mx-5">
                <div className="flex justify-center items-center">
                    <div className="w-full">
                        ...
                        <div className="mt-5">
                            {
                                PurchaseListByDate.length>0?(
                                    <div className="shadow-lg border w-5/6 h-auto bg-white rounded-md p-5">
                                        <div className="flex justify-center items-center bg-green-50 py-2 text-lg font-semibold">
                                            <h2>Download Report</h2>
                                        </div>
                                        <div className="mt-5 font-semibold">
                                            <h6>Total:<span className="text-gray-700"> {currencyFormatter.format(PurchaseListByDate[0]['Total'][0]['TotalAmmount'],{code:"USD"})}</span></h6>
                                        </div>
                                        <div className="flex justify-center items-center p-8">
                                            <div className="flex space-x-6">
                                                <button onClick={()=>onExportData('csv',PurchaseListByDate[0]['Rows'])} className="bg-green-700 rounded-md px-5 py-1 text-white font-semibold ring-2 ring-green-500 hover:bg-green-600 hover:ring-green-800 ">Download CSV</button>
                                                <button onClick={()=>onExportData('xls',PurchaseListByDate[0]['Rows'])} className="bg-green-700 rounded-md px-5 py-1 text-white font-semibold ring-2 ring-green-500 hover:bg-green-600 hover:ring-green-800 ">Download XLS</button>
                                            </div>
                                        </div>
