...
import {SaleReportByDateRequest} from "../../APIRequest/ReportAPIRequest.js";
const SaleReport = ()=> {
    const FromDateRef = useRef();
    const ToDateRef = useRef();

    const SalesListByDate=useSelector((state)=>state.Report.SalesByDateList)
    const CreateReport = async () => {
        ...
        if (isEmpty(FromDate)) {
            ...
            await SaleReportByDateRequest(FromDate,ToDate);
            ...
            <div className="container mx-5">
                <div className="flex justify-center items-center">
                    <div className="w-full">
                        ...
                        <div className="mt-5">
                            {
                                SalesListByDate.length>0?(
                                    <div className="shadow-lg border w-5/6 h-auto bg-white rounded-md p-5">
                                        <div className="flex justify-center items-center bg-green-50 py-2 text-lg font-semibold">
                                            <h2>Download Report</h2>
                                        </div>
                                        <div className="mt-5 font-semibold">
                                            <h6>Total:<span className="text-gray-700"> {currencyFormatter.format(SalesListByDate[0]['Total'][0]['TotalAmount'],{code:"USD"})}</span></h6>
                                        </div>
                                        <div className="flex justify-center items-center p-8">
                                            <div className="flex space-x-6">
                                                <button onClick={()=>onExportData('csv',SalesListByDate[0]['Rows'])} className="bg-green-700 rounded-md px-5 py-1 text-white font-semibold ring-2 ring-green-500 hover:bg-green-600 hover:ring-green-800 ">Download CSV</button>
                                                <button onClick={()=>onExportData('xls',SalesListByDate[0]['Rows'])} className="bg-green-700 rounded-md px-5 py-1 text-white font-semibold ring-2 ring-green-500 hover:bg-green-600 hover:ring-green-800 ">Download XLS</button>
                                            </div>
                                        </div>
