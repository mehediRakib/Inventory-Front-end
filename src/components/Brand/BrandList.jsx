...
const BrandList = () => {
    ...
    useEffect( () => {
        ...
        (async () => {
            await brandListRequest(1, perPage, searchArray);
        })()

    }, []);

    const TotalProduct=useSelector((state)=>(state.brand.brandListTotal));
    const ProductList=useSelector((state)=>state.brand.List);

    const handlePageClick=async (e) => {
        await brandListRequest(e.selected+1,perPage,searchArray);
    }
    const perPageOnChange=async (e) => {
        setPerPage(parseInt(e.target.value))
        await brandListRequest(1,e.target.value,searchArray)
    }
                                     ...
                                     <tr className="bg-white border-b">
                                         ...
                                         <td className="px-4 py-2">
                                             <div className="flex space-x-5 items-center">
                                                 <Link to={`/CreateUpdate-BrandList?id=${item['_id']}`}>
                        ...
                        <div className="mt-10">
                            <nav aria-label="Page navigation">
                                <ReactPaginate
                                    ...
                                    pageCount={Math.ceil(TotalProduct / perPage)}
