...
import {DeleteExpenseTypeRequest, expenseTypeApiRequest} from "../../APIRequest/ExpenseTypesApIRequest.js";
...
const ExpenseTypeList =() => {
    ...
    useEffect( () => {
        ...
        (async () => {
            await expenseTypeApiRequest(1, perPage, searchArray);
        })()

    }, []);

    const TotalExpenseType=useSelector((state)=>(state.expenseType.TotalExpenseTypeList));
    const ListOfExpenseType=useSelector((state)=>state.expenseType.List);

    const handlePageClick=async (e) => {
        await expenseTypeApiRequest(e.selected+1,perPage,searchArray);
    }
    const perPageOnChange=async (e) => {
        setPerPage(parseInt(e.target.value))
        await expenseTypeApiRequest(1,e.target.value,searchArray)
    }
    const searchKeywordOnChange=async (e)=>{
        setSearchArray(e.target.value);
        if((e.target.value).length===0){
            setSearchArray('0');
            await expenseTypeApiRequest(1,perPage,'0');
        }
    }
    ...
    const doDelete=async (id)=>{
        ...
        if(res.isConfirmed){
            const result=await DeleteExpenseTypeRequest(id);
            if(result){
                await expenseTypeApiRequest(1, perPage, searchArray);
            }
...

                                {
                                    ListOfExpenseType.map((item,index)=>
                                        <tr className="bg-white border-b">
                                            ...
                                            <td className="px-4 py-2"> {new Date(item['createDate']).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })} </td>
                                            <td className="px-4 py-2">
                                                <div className="flex space-x-5 items-center">
                                                   <Link to={`/Create-Update-expenseType?id=${item['_id']}`}>
